...
<script type="text/template" id="noList">
	<div class="err"> <i class="spr_book ico_info_nolist"></i>
						<h1 class="tit">예약 리스트가 없습니다</h1>
					</div>
</script>
<script type="text/template" id="lastList">
	<article class="card_item">
	<a href="#" class="link_booking_details">
										<div class="card_body">
											<div class="left"></div>
											<div class="middle">
												<div class="card_detail">
													...
													<div class="booking_cancel">
														<a href="reviewWrite?displayInfoId={{reservationInfoId}}&reservationInfoId={{reservationInfoId}}"><button class="btn"><span>예매자 리뷰 남기기</span></button></a>
													</div>
												</div>
											</div>
											<div class="right"></div>
    ...
    <script type="text/template" id="confirmList">
<article class="card_item">
								<a href="#" class="link_booking_details">
									<div class="card_body">
										<div class="left"></div>
    ...
    <script type="text/template" id="cancelList">
		<article class="card_item">
			<a href="#" class="link_booking_details">
				<div class="card_body">
				<div class="left"></div>
				...
				<span class="item_tit">장소</span>
					...
					<span class="item_tit">업체</span>
					<em class="item_dsc">
						{{displayInfo.productDescription}}</em></li></ul>
					<div class="price_summary">
				<span class="price_tit">결제 예정금액</span>
				<em class="price_amount">
				<span>{{totalPrice}}</span>
			...
			<div class="right"></div></div><div class="card_footer"><div class="left"></div><div class="middle"></div>
<div class="right"></div></div></a></article>		
    </script>		
<script>
...
function getMyreservation(){
	fetch('api/reservations/?reservationEmail='+reservationEmail, {
        method: 'get',
        headers: {},
...
function setData(data){
	...
	data.map(i=>{
		i.totalPrice = i.totalPrice.toLocaleString();
		if(i.cancelYn===true) cancel.push(i);
		else if(new Date(i.reservationDate)>nowDate){
			confirm.push(i);
		}else last.push(i);
	});
	let node = document.querySelector(".summary_board").getElementsByClassName("figure");
	//전체 
	node[0].innerText = data.length;
	node[1].innerText = confirm.length;
	node[2].innerText = last.length;
	node[3].innerText = cancel.length;
	console.log(cancel, confirm, last);

	setTemplate(confirm, "confirmList", 1);
	setTemplate(last, "lastList", 2);
	setTemplate(cancel, "cancelList", 3);
}
...
function setTemplate(data, className, index){
	var template = document.querySelector("#"+className).innerText;
	if(data.length == 0 ) {
		template = document.querySelector("#noList").innerText;
		let n = document.querySelector(".list_cards").children[index].innerHTML;
		document.querySelector(".list_cards").children[index].innerHTML =n+template;
		return;
	}
	var bindTemplate = Handlebars.compile(template);
	var resultHtml = data.reduce(function(prev, next) {
		next.reservationDate = dateFormat(next.reservationDate);
		return prev + bindTemplate(next);
	}, "");
	var node = document.querySelector(".list_cards").children[index];
